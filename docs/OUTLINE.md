...
- Output feeds Table 3 (Delivery and automation) at 4:15. Each table produces one takeaway and each attendee a 30-day commitment, so the talk must leave something usable.
...
21 slides, about 27 minutes, one accent per slide. SLOW marks where Tim spends the time he saves by running fast.
...
3. The typing is hiding a decision (illustrated, orange). SLOW. The insight the deck rests on. A belt of identical gray boxes, one lid open on a small orange part (generated image, docs/concepts/conveyor-1.png). Copy the name, open the PSA, find the client, pick the build, fill the fields, save: five minutes a computer could do. One step is not a step: pick the build is a decision, standard unless it is the client that always gets the exception. Five seconds of judgment inside five minutes of typing, too small for anyone to have noticed. The typing cannot leave the person until the decision leaves their head. The tech is not paid to type; the tech is paid to be where the rule lives while the typing happens. That is why you became the automaton. Bridge: how did the rule end up in a head instead of on paper?
...
5. The demo never had a client (two-column). Every AI keynote is one sentence then magic; it works because the presenter has no stake in which game he gets. Then you go home and ask for a client email where every detail is the job. No vendor names. Twenty seconds.
...
20. One action (closing). Pick one of the six. Sit with your best tech for an hour. Write down everything they fill in that you never said. That is your definition of done. Bring the page to Table 3. Hand to Erik and Atul: the version where the person the process lives in is you.
21. Presenter.
